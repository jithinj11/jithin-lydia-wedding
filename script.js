(()=>{
  ...
  function show(n){
    ...
    scenes[n]?.scrollTo?.({top:0,behavior:'instant'});
  ...
  function isEventSceneActive(){const a=scenes[i];return !!a&&a.classList.contains('event-scene')}
  function blockScroll(e){if(playing&&!isEventSceneActive())e.preventDefault()}
  ...
  addEventListener('wheel',e=>{if(playing&&isEventSceneActive()){e.stopPropagation();return}blockScroll(e)},{passive:false});
  addEventListener('touchmove',e=>{if(playing&&isEventSceneActive()){e.stopPropagation();return}blockScroll(e)},{passive:false});
